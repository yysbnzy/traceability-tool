...
2. 拼接配置（可选）
   • 用例ID/需求ID可添加前缀/后缀
   • 支持固定文本或从Excel列读取
   • 仅双向溯源表显示拼接后的ID，其他表使用原始ID

3. 需求文档（可选但建议）
   • 导入独立的需求清单可计算覆盖率
   • 识别未覆盖的孤儿需求
   • 拼接来源选"需求文档列"时需先上传需求文档

4. 过滤配置（可选）
   • 用例ID过滤：只分析指定前缀的用例（匹配原始ID）
   • 需求ID过滤：只分析指定前缀的需求（匹配原始ID）
   • 过滤仅影响双向溯源表显示，不影响异常分析
...
• 双向溯源表-合并版：用例与需求的双向关联（合并单元格显示）
• 双向溯源表-逗号版：用例与需求的双向关联（逗号分隔显示）
• 输入源：原始数据的完整副本（原始ID，不拼接）
• 异常分析：孤儿用例、孤儿需求、被过滤项（原始ID，不拼接）
• 需求缺失用例：需求文档有但用例未覆盖（原始ID）
• 统计汇总：覆盖率等关键指标
...
Q: 过滤条件匹配不到数据？
A: 过滤条件匹配的是原始ID（不含前缀/后缀），请用原始ID前缀过滤

Q: 双向溯源表ID和其他表不一致？
A: 这是设计意图——双向溯源表显示拼接后的ID用于展示，其他表保留原始ID用于核对
"""
    ...
    def export(self, output_path, case_to_reqs, case_to_reqs_raw, req_to_cases,
        ...
        for idx, case_id in enumerate(sorted_cases, start=2):
            ...
            ws_comma.cell(row=idx, column=2, value=", ".join(display_reqs))
            for c in [1, 2]:
        ...
        for idx, req_id in enumerate(sorted_reqs, start=2):
            ...
            ws_comma.cell(row=idx, column=5, value=", ".join(display_cases))
            for c in [4, 5]:
                ws_comma.cell(row=idx, column=c).border = thin_border
                ws_comma.cell(row=idx, column=c).alignment = left_align
