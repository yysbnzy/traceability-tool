...
def analyze_traceability(case_file):
    ...
    # 加载用例文档
    print(f'\n[1] 加载用例文档: {case_file}')
    case_wb = load_workbook(case_file, read_only=True, data_only=True)
    case_ws = case_wb.active
    print(f'    工作表名称: {case_ws.title}')
    print(f'    数据范围: {case_ws.dimensions}')
    ...
    # 导出结果到Excel
    print('\n[10] 导出分析结果...')
    export_results(case_to_reqs, req_to_cases, orphan_cases, case_file)
    
    print('\n' + '='*80)
    print('分析完成!')
    print('='*80)

def export_results(case_to_reqs, req_to_cases, orphan_cases, original_file):
    ...
    for idx, req_id in enumerate(sorted(req_to_cases.keys()), start=2):
        ...
        ws1.cell(row=idx, column=5, value=", ".join(sorted(cases)) if cases else "(无关联用例)")
