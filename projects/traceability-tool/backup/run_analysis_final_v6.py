# -*- coding: utf-8 -*-
"""
测试用例与需求溯源分析 - 最终版 v6 (按用户要求的工作表顺序)
工作表顺序:
1. 双向溯源
2. 输入源
3. 差异分析
...
def analyze_traceability(case_file=None):
    ...
    if case_file is None or not os.path.exists(case_file):
        print("错误: 未找到Excel文件！")
        return
    
    print('='*80)
    print('测试用例与需求溯源分析工具 - 最终版 v6')
    print('='*80)
    ...
    print(f'    用例ID列: 索引 {case_id_col}')
    print(f'    需求ID列: 索引 {req_id_col}')
    ...
    # 导出结果
    print('\n[3] 导出分析结果...')
    export_results(case_to_reqs, req_to_cases, orphan_cases, orphan_reqs, raw_data, 
                   case_file, headers, case_id_col, req_id_col)
    
    print('\n' + '='*80)
    print('分析完成!')
    print('='*80)

def export_results(case_to_reqs, req_to_cases, orphan_cases, orphan_reqs, raw_data, 
                   original_file, headers, case_id_col, req_id_col):
    output_file = original_file.replace('.xlsx', '_溯源分析结果_最终版.xlsx')
    ...
    for idx, (case_id, req_text) in enumerate(raw_data, start=2):
        ws2.cell(row=idx, column=1, value=case_id if case_id else '(空)')
        ws2.cell(row=idx, column=2, value=req_text if req_text else '(空)')
    ...
    for req_id in orphan_reqs:
        ...
        ws3.cell(row=row_idx, column=3, value="该需求在输入文档中存在，但没有关联任何测试用例")
        for c in [1, 2, 3]:
            ws3.cell(row=row_idx, column=c).border = thin_border
        row_idx += 1
    ...
    # ========== Sheet5: 统计 ==========
    ws5 = wb.create_sheet("统计")
    ws5['A1'] = '统计项'
    ws5['B1'] = '数值'
    for cell in [ws5['A1'], ws5['B1']]:
        cell.font = header_font
        cell.fill = header_fill
        cell.border = thin_border
...
def find_excel_file():
    ...
    if len(excel_files) == 0:
        ...
        for f in excel_files:
            if '用例' in f or '需求' in f:
                return os.path.join(current_dir, f)
        return os.path.join(current_dir, excel_files[0])

if __name__ == "__main__":
    case_file = sys.argv[1] if len(sys.argv) > 1 else None
